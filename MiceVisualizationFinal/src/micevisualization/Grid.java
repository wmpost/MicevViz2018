...
public class Grid {
    ...
    Grid() {
        ...
        */
        ...
        SingleMiceBehaviours node3 = new SingleMiceBehaviours("inactive", 2, "equal");
        SingleMiceBehaviours node4 = new SingleMiceBehaviours("inactive", 4, "equal");
        //SingleMiceBehaviours node9 = new SingleMiceBehaviours("inactive", 5, "equal");
        SingleMiceBehaviours node10 = new SingleMiceBehaviours("drinking", 21, "equal");
        SingleMiceBehaviours node5 = new SingleMiceBehaviours("drinking", 23, "equal");
        SingleMiceBehaviours node6 = new SingleMiceBehaviours("eating", 25, "equal");
        SingleMiceBehaviours node8 = new SingleMiceBehaviours("wheel", 31, "equal");
        ...
        Node inactive2 = new Node(node3);
        Node inactive4 = new Node(node4);
        //Node inactive5 = new Node(node9);
        Node drinking1 = new Node(node10);
        Node drinking2 = new Node(node5);
        ...
        rootNode.addChild(drinking1); //add drinking to rootNode
        rootNode.addChild(drinking2); //add drinking to rootNode
        rootNode.addChild(inactive4); //add inactive1 to rootNode
    ...
    void drawSectorsNumbers(Canvas c) {
        ...
        for (int i = 0; i < this.sectors.size(); ++i) {
            if (this.sectors.get(i).gridIndex != 0) {
    ...
    void redraw(StackPane viewerPane, Boolean showGridNumbers, Boolean showGridLines, Boolean isGenerated) {
        ...
        */
//begin new Checkerboard code WP 11/7/17
        int ROWS = 4;
        int COLS = 13;
        ...
        for (int i = 0; i < ROWS; ++i) {
            ...
            for (int j = 0; j < COLS-1; ++j) { // handle the first 24 grid sectors (#s 0 - 23)
                currentX = (width / COLS) * j;
                if(i % 2 == 0){ //Row Zero or Even Row
                    if (j % 2 == 0) { //Column Zero or Even Column
                        gs = new GridSector(currentX, currentY, width / COLS, height / ROWS, 0);
                    }
                    else { // Odd Column
                        gid = ((i+j+1)*2)+evenbase;
                        gs = new GridSector(currentX, currentY, width / COLS, height / ROWS, gid);
                    }
                }
                else { //Odd Row
                    ...
                    // unitLabel (the gridSector id# from the data set) ranges from 1 - 25, hence (j*4 + i) + 1
                this.addSector(gs);
            }//end for
            if (i % 2 == 0)
                evenbase = evenbase - 5;
        ...
        gs = new GridSector((width / COLS) * 12, height/2 - ((height/ROWS)/2), width/COLS, height/ROWS, 25);
        this.addSector(gs);
        ...
        for (int i = 0; i < mice.size(); ++i) {
            for (int j = 0; j < mice.get(i).locTimeData.size() && mice.get(i).locTimeData.get(j).timestamp.compareTo(stop) <= 0; ++j) {
                ...
                if (mouseDate.compareTo(start) >= 0) {
                    ...
                    int gridSectorIndex = mice.get(i).locTimeData.get(j).unitLabel;
                    ...
                    if(isRewind){
                        for (int j = overallfinalJ; j > 0; --j) {
                            ...
                            Platform.runLater(() -> {
                                ...
                                int gridSectorIndex = locTimeData.get(finalJ).unitLabel;
     ...
     * @throws InterruptedException 
     */
    void animatedOverlayMap(StackPane viewerPane, Button generateButton, TextArea currentAnimationFrame, Label leftStatus, ArrayList<Mouse> mice, Date start, Date stop, double speed, Color[] ColorList, AppStageController controller, File exportFolder, boolean rewind) throws InterruptedException {
        // (Parker 4/3/17): calculate the maximumDuration of mouse activity in the most active GridSector,
        // and calculate each GridSector's finalTotalDuration:
        isRewind = rewind;
        double maxDuration = calculateGridSectorHeatMapInfo(viewerPane, mice, start, stop);
                           ...
                           // we need to make a new final variable that is a copy of j's current value:
                            ...
                            if (mouseDate.compareTo(start) >= 0) {
                                ...
                                Platform.runLater(() -> {
                                    ...
                                    /* perform string manipulation to get the integer gridIndex from the unitLabel locTimeData parameter: */
                                    int gridSectorIndex_previous = locTimeData.get(finalJ).unitLabel;
                                    ...
                                    int gridSectorIndex_current = locTimeData.get(nextIndex).unitLabel;
                                    /* use the gridIndex to retrieve the matching gridSector object from the grid's gridSector array: */
                                    ...
                                    current_mouse_positions.entrySet().stream().filter((entry) -> (entry.getValue()!=null)).map((entry) -> {
                                        ...
                                        vectorCanvasContext.fillOval((entry.getKey().x * (entry.getValue().center_x - entry.getValue().x)) + entry.getValue().x, (entry.getKey().y * (entry.getValue().center_y - entry.getValue().y)) + entry.getValue().y, 10, 10);
                                    });
                                ...
                                Platform.runLater(() -> {
                                    ...
                                    for (int i = 0; i < ColorList.length; i++) {
                                        if (currentColor.equals(ColorList[i])) {
                                            ...
                                            int location = locTimeData.get(finalJ).unitLabel;
                        ...
                        for (int j = overallfinalJ; j > 0; --j) {
                            ...
                            if (mouseDate.compareTo(start) >= 0) {
                                ...
                                Platform.runLater(() -> {
                                    ...
                                    //get the integer gridIndex from the unitLabel locTimeData parameter:
                                    int gridSectorIndex_previous = locTimeData.get(finalJ).unitLabel;
                                    int nextIndex = finalJ+1;
                                    // (Parker): prevent an index out of bounds error:
                                    if (nextIndex >= locTimeData.size()) nextIndex = finalJ;
                                    ...
                                    current_mouse_positions.entrySet().stream().filter((entry) -> (entry.getValue()!=null)).map((entry) -> {
                                        ...
                                        dataCanvasContext.fillOval((entry.getKey().x * (entry.getValue().center_x - entry.getValue().x)) + entry.getValue().x, (entry.getKey().y * (entry.getValue().center_y - entry.getValue().y)) + entry.getValue().y, 10, 10);
                        ...
                        for (int j = overallfinalJ; j+1 < locTimeData.size() && locTimeData.get(j).timestamp.compareTo(stop) <= 0; ++j) {
                            ...
                            if (mouseDate.compareTo(start) >= 0) {
                                ...
                                Platform.runLater(() -> {
                                    ...
                                    //Hopefully see if mice are being chased.
                                    ArrayList<String> chasing = new ArrayList<>();
                                    for (Neighbor n: locTimeData.get(finalJ).neighbors) {
                                        if (gridSectorIndex_previous == n.getGrid())
                                            chasing.add(n.getMouseID());
                                    }
                                    if(!chasing.isEmpty()){
                                        currentColor = mouse_color.get(locTimeData.get(finalJ).timestamp).mouse_color;
                                        for (int i = 0; i < ColorList.length; i++) {
                                            if (currentColor.equals(ColorList[i])) {
                                                controller.updateBehaviour("chased", i);
                                                break;
                                            }
                                        }
                                    }

...
public class AppStageController {
    ...
    protected SimpleDateFormat sdf = new SimpleDateFormat("MM/dd/yyyy HH:mm:ss.SSS"); // (Parker): define a string format for converting Dates to Strings:
    ...
    mouseProximity prox = new mouseProximity(); //Object for mouseProximity class
                    ...
                    switch (mapTypeChoiceBox.getValue().toString()){
                        ...
                        case "Vector":
                           grid.animatedVectorMap(viewerPane, generateButton, currentAnimationFrame, leftStatus, selectedMice, startIndex, stopIndex, animationSpeedSlider.getValue(),
                            ...
                            break;
                        case "Overlay":
                            grid.animatedOverlayMap(viewerPane, generateButton, currentAnimationFrame, leftStatus, selectedMice, startIndex, stopIndex, animationSpeedSlider.getValue(),  ColorList, this, exportFolder,false);
                            break;
                        default:
                            break;
                    }//end switch
    ...
    */
    ...
    @FXML protected void openFileAction(ActionEvent event) throws URISyntaxException, IOException {
        ...
        fileChooser.setTitle("Select a data set or session file");
        fileChooser.setInitialDirectory(
            new File(System.getProperty("user.home"))
        );
        // prepopulate the file chooser with allowable file extensions:
        fileChooser.getExtensionFilters().addAll(
            new FileChooser.ExtensionFilter("DataSet (CSV)", "*.csv"),
                    ...
                    // so proceed to generate one:
                        ...
                        switch (mapTypeChoiceBox.getValue().toString()) {
                            ...
                            case "Overlay":
                                ...
                                //Jacqueline:(6/13/17) enable pause button
                                pauseButton.setDisable(false);
                                grid.animatedOverlayMap(viewerPane, generateButton, currentAnimationFrame, leftStatus, selectedMice, startIndex, stopIndex, animationSpeedSlider.getValue(), ColorList,this, null,true);
                                this.rewind.setText("Continue rewind");
                                break;
                                }
                                pauseButton.setDisable(false);
                                grid.animatedOverlayMap(viewerPane, generateButton, currentAnimationFrame, leftStatus, selectedMice, startIndex, stopIndex, animationSpeedSlider.getValue(), ColorList,this, null,false);
                            default:
                                break;
                        }//end switch
                    }//end else
    ...
    @FXML protected void totalTimeInRangeAction(ActionEvent event) throws InterruptedException{
        ...
        Removed confusing, complicated request for DTS range and instead use start and stop ranges from
        Text controls in the visualization options.*/
        if (!startDataRangeTextArea.getText().isEmpty() && !stopDataRangeTextArea.getText().isEmpty())
            getTotalTimeInRange(startDataRangeTextArea.getText(),stopDataRangeTextArea.getText());
    }
    /**
     * 
     * author: parker
     ...
     * to go either here or in the MouseViz.java file's stop() function.
     * 
     * @param event 
     */
    @FXML
    ...
    private Boolean openFile(File file) {
        ...
        long fileStartTime;
        leftStatus.setText("Opening " + file.getName() + " ...");  // update the status text

        // create the necessary FileInputStream and Scanner objects for reading the file:
        FileInputStream inputStream = null;
        Scanner sc = null;

        int linesProcessed = 0;  // track how many lines have been processed during the data parsing operation:
        ArrayList<CSVLine> csvLineArrayList = new ArrayList<>();
        CSVLine l;
        ...
        try {  // attempt to open the file for reading
            inputStream = new FileInputStream(file.getPath());
            sc = new Scanner(inputStream, "UTF-8");

            fileStartTime = System.currentTimeMillis();  // start timing the file processing action
            ...
            if (extension.equals("csv")) {    // process .csv data files
                ...
                selectedMiceIndices = session.selectedMiceIndices;

                grid = new Grid(); // reset the grid object
                mice = new Mice(); // reset the mice object
                ...
                while (sc.hasNextLine()) {
                    ...
                    l = new CSVLine((line));
                    if (l.getDTS() != null) csvLineArrayList.add(l);
                }
                Collections.sort(csvLineArrayList);
                int foundAt;
                int mouseCount = 0;
                for (CSVLine csv : csvLineArrayList) { //Loop through all valid input lines
                    foundAt = -1;
                    ...
                    if (!miceTimes.isEmpty()) {
                        for (int i = 0; i < miceTimes.size(); i++) {
                            if (csv.getMouseLabel().equalsIgnoreCase(miceTimes.get(i).getName())) {
                                foundAt = i;
                                miceTimes.get(foundAt).incrementTime(csv.getGridID(), csv.getDuration());
                                break;
                            }
                        }
                    }
                    if (foundAt == -1) {
                        miceTimes.add(new MouseByTotalTime(csv.getMouseLabel()));
                        miceTimes.get(miceTimes.size() - 1).incrementTime(csv.getGridID(), csv.getDuration());
                    }
                    ...
                    dataTimestampsList.add(csv.getDTS());
                    MouseLocTime mlt;
                    ArrayList<Neighbor> myNeighbors;

                    if (mice.hasMouse(csv.getMouseID()) == false) { // if the current mouse in the data set does not have a corresponding Mouse object, create one:
                        myNeighbors = mice.getNeighbors(csv.getMouseID()); // create Neighbor ArrayList for MLT row
                        mlt = new MouseLocTime(csv.getDTS(), csv.getGridID(), csv.getDuration(), myNeighbors); //Create MLT row
                        Mouse m = new Mouse(csv.getMouseID(), csv.getMouseLabel(), ColorList[mouseCount],
                                location[mouseCount][0], location[mouseCount][1], csv.getGridID(), csv.getDTS());
                    ...
                    // get the Mouse object by IdRFID and add the current row's location and timestamp data:
                        ...
                        myNeighbors = mice.getNeighbors(csv.getMouseID());
                        mlt = new MouseLocTime(csv.getDTS(), csv.getGridID(), csv.getDuration(), myNeighbors); //Create MLT row
                        mice.getMouseByIdRFID(csv.getMouseID()).addLocTime(mlt);
                    }
                }//end for each loop

                //Theoretically finished reading file/making mice, so set fileEndTime
                long fileEndTime = System.currentTimeMillis();

                //mice.print();

                //Create Mouse Proximity Spreadsheet
                prox.makeProxXLSX(mice, miceTimes);

                Collections.sort(csvLineArrayList); //Re-sort list just in case before updating GUI controls
                populateFields(csvLineArrayList.get(0).getDTS(), dataTimestampsList); //Update GUI controls with DTS vaules

                // file    processing finished; calculate the time spent:
                long elapsedTime = fileEndTime - fileStartTime;
                System.out.println("Done reading file! It took " + elapsedTime + " milliseconds");
                System.out.println("Lines Processed = " + linesProcessed);
                leftStatus.setText("Finished opening " + file.getName() + " in " + elapsedTime + " milliseconds.");
                return true;
            }
    ...
    private void populateFields(Date startDate, ArrayList<Date> dataTimestampsList)
    {
        ...
        if (session.stoppingIndex.equals(""))
            startDataRangeTextArea.setText(sdf.format(startDate));
    ...
    public void getTotalTimeInRange(String start, String stop){
        ...
        try{
            ...
            ArrayList<MouseByTotalTime> miceTimes = new ArrayList<>();
            makeSelectedExportFile(selectedMice, locTimeData, miceTimes, startDate, endDate);
        }
        catch(Exception e){
            Alert alert = new Alert(AlertType.ERROR);
            alert.setTitle(null);
            alert.setHeaderText("Please make your start and stop selections");
            alert.show();
        }
    }

    public void makeSelectedExportFile(ArrayList<Mouse> selectedMice, ArrayList<MouseLocTime> locTimeData, ArrayList<MouseByTotalTime> miceTimes, Date startDate, Date endDate){
        for(int i = 0; i < selectedMice.size(); i++){
            ...
            for(int j = 0; j < selectedMice.get(i).locTimeData.size(); j++){
                if (selectedMice.get(i).locTimeData.get(j).timestamp.compareTo(endDate) <= 0) {
                    if (selectedMice.get(i).locTimeData.get(j).timestamp.compareTo(startDate) >= 0) {
                        miceTimes.get(i).incrementTime(selectedMice.get(i).locTimeData.get(j)
                                .unitLabel,selectedMice.get(i).locTimeData.get(j)
                                .eventDuration);
                    }
                }
            }
        ...
        try {
            File f = prox.showFileDialog(prox.SAVE, "Save Mouse Times in Range", prox.EXCELFILE, prox.XLSFILES);
            if (f != null) {
                FileOutputStream out = new FileOutputStream(f);
                workbook.write(out);
                out.close();
                System.out.println("Mouse Times Range Written.");
            }
        }
        catch (Exception e){
